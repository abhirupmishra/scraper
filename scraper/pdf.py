...
from scraper.scraper import Scraper, ScrapeResult
...
def compute_filtered_tokens(text: list[str]) -> set[str]:
    ...
    Returns:
        tokens (set): set of filtered tokens
    """
    word_tokens = word_tokenize("\n".join(text))
    return set([w for w in word_tokens if w not in STOP_WORDS and NAME_WORDS])
...
class PDFScraper(Scraper):
    ...
    def scrape(self, search_text: str) -> ScrapeResult:
        """
        scrape method implementation for pdf scraper
